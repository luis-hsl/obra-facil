...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {/* Sidebar desktop */}
      <aside className="hidden md:flex fixed left-0 top-0 h-screen w-60 bg-white border-r border-slate-200 flex-col z-10 shadow-sm">
          ...
          {navItems.map((item) => {
            ...
            return (
              <a
                key={item.href}
                href={item.href}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium no-underline mb-0.5 ${
                  isActive
        ...
        <div className="px-3 pb-5 border-t border-slate-100 pt-3">
          <button
            onClick={signOut}
            className="w-full flex items-center gap-3 text-left px-3 py-2.5 rounded-xl text-sm text-slate-500 hover:bg-slate-50 hover:text-slate-700"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {BOTTOM_NAV_MORE.map((item) => (
                <a
                  ...
                  className={`flex items-center gap-3 px-4 py-2.5 text-sm font-medium no-underline ${
                    item.match(pathname) ? 'text-blue-600 bg-blue-50' : 'text-slate-700 hover:bg-slate-50'
                  }`}
                >
                  {item.icon}
                  {item.label}
          ...
          {BOTTOM_NAV_MAIN.map((item) => {
            ...
            return (
              <a
                key={item.href}
                href={item.href}
                className={`flex flex-col items-center py-2 px-1 min-w-0 flex-1 no-underline relative ${
